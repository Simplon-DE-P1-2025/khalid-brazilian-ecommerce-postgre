        ...
        for table, pk_col in pks.items():
            if pk_col:
                try:
                    ...
                    print(
                        f"   ⚠️  Impossible d'ajouter PK sur {table} (déjà existante ou doublons ?)"
                    )
    ...
    with engine.connect() as conn:
        for table, col, ref_table, ref_col in fks:
            try:
                ...
                query = text(
                    f"ALTER TABLE {table} ADD CONSTRAINT {constraint_name} FOREIGN KEY ({col}) REFERENCES {ref_table} ({ref_col});"
                )
                conn.execute(query)
                conn.commit()
                ...
                print(
                    f"   ⚠️  FK échouée : {table}.{col} (Données orphelines ou contrainte déjà là)"
                )
